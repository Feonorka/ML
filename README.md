...
3. [Корреляция](https://www.helenkapatsa.ru/korrieloghramma/ "Коррелограмма (Correlogram)"). Определиться с поиском наиболее значимых параметров.
4. Примеры данных
...
# Урок 9
### Задача МО:
Получение **целевого значения** - некоторого идеального состояния данных, идеального предсказания, которое данные должны принять. Это может быть либо среднее значение, дибо некоторое ожидаемое значение.
### Проблема модели МО:
1. Данное значение в данных отсутствует.
2. Модель машинного обучения смещается относительно этого предсказания.
### Цель инженера МО:
Построить модель, которая будет иметь небольшую величину смещения.
## Смещение
Расстояние между математическим ожиданием данных и математическим ожиданием самой модели. Принято называть его **bias**

## Разброс
Отклонение предсказаний от математического ожидания самой модели
## Ошибка данных
Результат суммы **статистического шума** и **погрещности**

[Статистический шум](https://www.helenkapatsa.ru/shum/?ysclid=lezjn60rs4312161991 "Шум (Noise)") - случайность измерений в определенных границах математическое ожидание плюс минус корень из дисперсии величины.
### Дисперсия:
1. <p align="left">Определение среднего значения</ p>

$\overline{X}=\Sigma x/n$

2. <p align="left">Вычисление разницы в величине каждого наблюдения от среднего
3. <p align="left">Вычисление квадрата разницы
4. <p align="left">Вычисление суммы квадратов разницы
5. <p align="left">Нахождение среднего значения квадратов разницы (**Дисперсия**)
6. <p align="left">Вычисление квадратного корня из дисперсии
...
$$m_{h+2}=1 + m_{h+1} + min{m_{h+1},m_h}$$
